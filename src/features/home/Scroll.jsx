...
const Scroll = ({movies,onLinkClick}) => {
    ...
    const newMovies = movies.map((movie) => (
        ...
        <img src={`http://image.tmdb.org/t/p/w500/${movie.poster_path}`} className=" rounded-xl opacity-60 hover:opacity-90 max-w-full object-contain"/>
    ...
    return (
        <>
             <div className="flex  justify-between items-center px-4 lg:px-6">
                ...
                <button className={`text-xs lg:text-[15px] text-gray-500 cursor-pointer hover:text-indigo-600 ${!all && 'self-end'}`} onClick={()=>{
                    setAll(prev=>!prev); 
                }}>
